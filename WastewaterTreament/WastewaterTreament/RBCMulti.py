...
C_in = 50.0          # Reduced inflow cyanide concentration (mg/L)
Q_out = 10.0         # Outflow rate (L/h)
V = 1000.0           # Volume of the RBC system (L)
mu_max = np.array([1.0, 0.8])  # Increased maximum specific growth rates (1/h)
K_s = np.array([10.0, 15.0])   # Half-saturation constants (mg/L)
k_d = np.array([0.01, 0.01])   # Reduced decay rates (1/h)
Y = np.array([1.0, 0.8])       # Increased yield coefficients (mg bacteria/mg cyanide)
Y_o = np.array([1.5, 1.2])     # Oxygen yield coefficients (mg oxygen/mg cyanide)
k_La = 50.0          # Increased oxygen transfer coefficient (1/h)
O_star = 8.0         # Saturation oxygen concentration (mg/L)

# Adjusted initial conditions
C0 = 0.0             # Initial cyanide concentration (mg/L)
X0 = np.array([50.0, 30.0])  # Increased initial bacterial concentrations (mg/L)
